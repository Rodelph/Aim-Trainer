...
{
	if (!this->bufferSfx.loadFromFile(this->ioFile->getSFXFile())) 
	{ 
		std::cout << this->ioFile->getSfxError() << std::endl; 
	}
	else 
	{ 
...
{
	if (!this->bgm.openFromFile(this->ioFile->getBGMFile())) 
	{ 
		std::cout << this->ioFile->getBgmError() << std::endl; 
	}
	else 
	{ 
		this->playBGM(); 
